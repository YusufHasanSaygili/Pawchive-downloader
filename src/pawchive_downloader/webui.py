...
class PawchiveHandler(BaseHTTPRequestHandler):
    ...
    def _start(self, body: dict[str, Any]) -> None:
        ...
        options = RunOptions(download=DownloadOptions(output=output,concurrency=concurrency,overwrite=bool(body.get("overwrite")),include_cover=bool(body.get("cover",True)),include_attachments=bool(body.get("attachments",True)),metadata=bool(body.get("metadata")),separate_post_folders=bool(body.get("postFolders"))),history_file=output/".pawchive-history.sqlite3",session_cookie=os.environ.get("PAWCHIVE_SESSION"),max_posts=limit)
        state.begin(output)
